...
class App extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.timerStatusChange, 1000)
  }
  ...
  onThumbnailItem = thumbnailUrl => {
    const {activeImgUrl} = this.state
    const filteredImages = imagesList.filter(
      eachImage => eachImage.thumbnailUrl === thumbnailUrl,
    )
    const {imageUrl} = filteredImages[0]
    if (imageUrl === activeImgUrl) {
      const newImgUrl =
  ...
  render() {
    const {activeImgUrl, activeTabId, score, limitTime, isTrue} = this.state
    const filteredThumbnailList = this.getFilteredThumbnailItems()
      ...
      <div className="app-container">
        <ul className="navbar-list">
          <NavBar
            score={score}
            limitTime={limitTime}
            onChange={this.timerStatusChange}
          />
        </ul>
        {!isTrue && (
          <div className="content-container">
            <div className="image-container">
              <img src={activeImgUrl} alt="match" className="img-url" />
              <ul className="tab-list-container">
                {tabsList.map(eachTab => (
                  <TabItem
                    key={eachTab.tabId}
                    tabDetails={eachTab}
                    onTabItem={this.onTabItem}
              ...
              <ul className="thumbnail-list-container">
                {filteredThumbnailList.map(eachImage => (
                  <ThumbnailItem
                    ...
                    onThumbnailItem={() =>
                      this.onThumbnailItem(eachImage.thumbnailUrl)
                    }
                  />
                ))}
              </ul>
